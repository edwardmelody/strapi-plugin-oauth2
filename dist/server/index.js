...
const errors = require("@strapi/utils/dist/errors");
const qs = require("qs");
const _interopDefault = (e) => e && e.__esModule ? e : { default: e };
const crypto__default = /* @__PURE__ */ _interopDefault(crypto$2);
...
function requireBufferEqualConstantTime() {
  ...
  var Buffer2 = require$$0__default.default.Buffer;
...
const jws$1 = jws$3;
const { KeyObject: KeyObject$1, createSecretKey: createSecretKey$1, createPublicKey } = crypto__default.default;
...
const jws = jws$3;
const { includes, isBoolean, isInteger, isNumber, isPlainObject, isString, once } = ___default.default;
const { KeyObject, createSecretKey, createPrivateKey } = crypto__default.default;
...
function getOAuthConfig() {
  ...
  jwtPublicKeyPath = path__default.default.join(process.cwd(), jwtPublicKeyPath);
  jwtPrivateKeyPath = path__default.default.join(process.cwd(), jwtPrivateKeyPath);
  if (jwtAlg === "RS256" && !fs__default.default.existsSync(jwtPublicKeyPath)) {
    throw new Error(`OAuth2 plugin: JWT public key file not found at path: ${jwtPublicKeyPath}`);
  } else if (jwtAlg === "RS256" && !fs__default.default.existsSync(jwtPrivateKeyPath)) {
    throw new Error(`OAuth2 plugin: JWT private key file not found at path: ${jwtPrivateKeyPath}`);
  }
  const jwtPublicKey = fs__default.default.readFileSync(jwtPublicKeyPath, "utf8");
  const jwtPrivateKey = fs__default.default.readFileSync(jwtPrivateKeyPath, "utf8");
  return {
    jwtAlg,
    jwtSignKey,
    accessTokenTTL,
...
async function hashSecret(secret) {
  const saltRounds = 12;
  return await bcrypt__default.default.hash(secret, saltRounds);
}
async function verifySecret(secret, hash) {
  return await bcrypt__default.default.compare(secret, hash);
}
...
function hashValue(value) {
  return crypto__default.default.createHash("sha256").update(value).digest("hex");
}
...
const { ValidationError: ValidationError$6, NotFoundError: NotFoundError$4, UnauthorizedError: UnauthorizedError$1 } = utils__default.default.errors;
...
const { ValidationError: ValidationError$5 } = utils__default.default.errors;
const oauthAccessToken$2 = strapi$1.factories.createCoreController(
          ...
          return await strapi2.db.transaction(async () => {
            ...
            if (client.clientType === "CONFIDENTIAL") {
              ...
              if (!validatedClient || validatedClient.id !== client.id) {
                throw new errors.UnauthorizedError("invalid_client_credentials", {
                  error: "invalid_client_credentials",
...
const { NotFoundError: NotFoundError$3, ValidationError: ValidationError$4 } = utils__default.default.errors;
const oauthClient$2 = strapi$1.factories.createCoreController("plugin::oauth2.oauth-client", ({ strapi: strapi2 }) => ({
  async find(ctx) {
    const filters = ctx.query?.filters || {};
    if (ctx.state.user && ctx.state.auth?.strategy?.name !== "admin") {
      ___default.default.set(filters, "user.documentId", ctx.state.user.documentId);
    }
...
const oauthGlobalSetting$2 = strapi$1.factories.createCoreController("plugin::oauth2.oauth-global-setting");
const { ValidationError: ValidationError$3, UnauthorizedError } = utils__default.default.errors;
const oauthAuthorizationCode$2 = strapi$1.factories.createCoreController(
  ...
  ({ strapi: strapi2 }) => ({
    async authorize(ctx) {
      ...
      try {
        ...
        if (!approve) {
          const q2 = qs__default.default.stringify({ error: "access_denied", state });
          return {
        ...
        const q = qs__default.default.stringify({ code: rawCode, state });
        return {
          redirectUri: `${redirectUri}?${q}`
        };
      } catch (err) {
        const q = qs__default.default.stringify({ error: err.message || "access_denied", state });
        return {
          redirectUri: `${redirectUri}?${q}`
        };
      }
...
const { ValidationError: ValidationError$2, NotFoundError: NotFoundError$2 } = utils__default.default.errors;
const oauthAccessToken = strapi$1.factories.createCoreService("plugin::oauth2.oauth-access-token", ({ strapi: strapi2 }) => ({
...
const { ValidationError: ValidationError$1, NotFoundError: NotFoundError$1 } = utils__default.default.errors;
const oauthClient = strapi$1.factories.createCoreService("plugin::oauth2.oauth-client", ({ strapi: strapi2 }) => ({
  ...
  async delete(documentId, params) {
    ...
    params = params || { ...ctx.query };
    ___default.default.set(params, "populate.user", true);
...
const oauthGlobalSetting = strapi$1.factories.createCoreService("plugin::oauth2.oauth-global-setting");
const { ValidationError, NotFoundError } = utils__default.default.errors;
const oauthAuthorizationCode = strapi$1.factories.createCoreService(
  "plugin::oauth2.oauth-authorization-code",
  ({ strapi: strapi2 }) => ({
