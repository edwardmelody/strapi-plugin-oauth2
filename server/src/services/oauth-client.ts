...
export default factories.createCoreService('plugin::oauth2.oauth-client', ({ strapi }) => ({
  ...
  async create(params) {
    ...
    const entity = await strapi.documents('plugin::oauth2.oauth-client').create({
      data: {
        ...data,
        clientId,
  ...
  async rotateClientSecret(documentId: string, userDocumentId: string) {
    ...
    const rawSecret = generateRawSecret(32);
    const secretHash = await hashSecret(rawSecret);
    await strapi.documents('plugin::oauth2.oauth-client').update({
  ...
  async delete(documentId, params) {
    ...
    const entity = await strapi.documents('plugin::oauth2.oauth-client').findOne({
      documentId,
      ...params,
    });
    ...
    return await strapi.db.transaction(async () => {
      await strapi.db.query('plugin::oauth2.oauth-access-token').deleteMany({
        where: {
          client: {
            documentId,
          },
      ...
      await strapi.db.query('plugin::oauth2.oauth-authorization-code').deleteMany({
        where: {
          client: {
            documentId,
          },
      ...
      await strapi.db.query('plugin::oauth2.oauth-user').deleteMany({
        where: {
          client: {
            documentId,
          },
  ...
  async generateKeyPair(clientDocumentId: string) {
    ...
    const client = await strapi.documents('plugin::oauth2.oauth-client').update({
